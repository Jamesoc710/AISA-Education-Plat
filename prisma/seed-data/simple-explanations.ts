/**
 * Simple explanations for all 57 concepts.
 *
 * Prompt guidance used to generate these:
 * "Write a 2-3 sentence explanation of [concept] for someone with zero
 * technical background. Use everyday analogies where possible. Avoid all
 * jargon. The reader has never taken a CS class."
 *
 * These are keyed by concept slug for easy lookup during seeding.
 * Output to prisma/seed-data/simple-explanations-review.json for manual review.
 */
...
export const SIMPLE_EXPLANATIONS: Record<string, string> = {
  ...
  "training-vs-inference-compute":
    "Training compute is the one-time cost of building an AI model (like constructing a building), while inference compute is the ongoing cost of running it for users (like the electricity bill). Training is a massive upfront investment that happens once; inference costs add up over time with every query users send. As AI becomes more popular, inference costs are becoming a bigger concern than training costs.",
